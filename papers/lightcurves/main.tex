...
transient is happens internally to surveys such as Pan-STARRS1
...
Besides the sheear amount of data, astronomical transients
...
These two characteristics (size and heteregoneity) have
...
Usually, new ML results come from gropus internal to a scientific
collaboration.
They have the levarage to make extensive use of the internal know-how,
and sometimes privileged access, on how the data is structured.
It is still difficult for other scientists to rebuild a training
dataset and perform comparisons with published results.  

Building traning datasets has been facilitated by the publication of
...
To help address the issue of making available datasets for ML
training, we compile and publish in easy-to-access files a dataset
that can be used to train and test different ML algorithms for transient detection.
